...
#define CNT_PER_REV 52953088.0    /* 實測:524288 enc x 101 gear */
#define MAX_CNT   ((int32_t)(10.0/360.0*CNT_PER_REV))  /* ±10 度輸出=可見 */
#define JOG_PERIOD 8.0
#define JOG_SECS   10.0
...
int main(int argc,char**argv){
    ...
    if(use_dc){
        ecrt_slave_config_dc(sc,0x0300,10000000,2000000,0,0);  /* SYNC0=10ms 對齊週期,shift 2ms */
    ...
    while(!s_stop && ph!=DONE && tick < 40000){          /* 上限 45 s */
        ...
        case JOG:
            cw=0x0F;
            tgt = start + (int32_t)(873813.3*t);  /* 100 rpm 馬達 = 1.667 rev/s */
            t += CYCLE_NS/1e9;
            if((ph_tick++%50)==0)
                printf("  t=%.1fs 命令Δ=%d cnt(%.2f馬達圈) | 實際Δ=%d cnt(%.4f馬達圈) foll=%d dcv=%umV sw=0x%04X\n",
                       t,tgt-start,(tgt-start)/524288.0,pos-start,(pos-start)/524288.0,
                       EC_READ_S32(pd+off_foll),EC_READ_U32(pd+off_dcv),sw);
