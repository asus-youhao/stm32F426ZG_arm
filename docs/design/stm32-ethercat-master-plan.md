...
| **SE1 SIL 基線報告** | `pc_master --bus ethercat @1kHz`（sim 後端）＋ trace ring 報表：p99、四相位 WCET、escalation 演練（掉軸/WKC 短少/DC 漂移注入） | SIL-B 報告一份（docs/changes），數字進 §8 預算表 | ⬜ |
| **SE2 vendor 搬入** | 從 linux-rt 分支帶 `third_party/SOEM`、`third_party/eyou_esi`、SAFEOP 工具；SOEM core 以 **arm-none-eabi 編過**（osal/oshw 先 stub） | arm build 連結成功（.map 檢視 Flash/RAM footprint） | ⬜ |
